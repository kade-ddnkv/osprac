...
int main(void)
{
  ...
  // Посылает разом 5 сообщений с типом 2.
  for (i = 1; i <= 5; i++) {
    mybuf.mtype = 2;
  ...
  // Принимает все сообщения с типом 1 до последнего.
  while (1) {
    maxlen = 512;

    msgrcv(msqid, (struct msgbuf *) &mybuf, maxlen, 1, 0);

    if (mybuf.msg.is_final_msg == true) {
      printf("second program exit\n");
      exit(0);
    }
